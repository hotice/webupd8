...
def main():
    ...
    while True:
        ...
        superkey_state = str(gsettings_get('org.compiz.unityshell', 
                                  '/org/compiz/profiles/unity/plugins/unityshell/', 
                                  'show-launcher'))
        altkey_state = str(gsettings_get('org.compiz.integrated', 
                                  '/org/compiz/integrated/', 
                                  'show-hud'))
        active_xid = str(screen.get_active_window().get_xid())
        wm_state =  run_cmd( ['xprop', '-root', '-notype','-id',active_xid, '_NET_WM_STATE'])  
        
        if debug : print(superkey_state,altkey_state,wm_state)

        if 'FULLSCREEN' in wm_state:
            if "Super" in  superkey_state:    
                gsettings_set('org.compiz.unityshell', 
                              '/org/compiz/profiles/unity/plugins/unityshell/',
                              'show-launcher', 
                              'Disabled')
            if "Alt" in altkey_state:
                # gsettings.set_strv doesn't work directly here so use workaround from http://bit.ly/2bO8vhf
                from subprocess import Popen, PIPE, STDOUT
                p = Popen(['dconf', 'load', '/org/compiz/integrated/'], stdout=PIPE, stdin=PIPE, stderr=STDOUT)
                p.communicate(input="[/]\nshow-hud=['disabled']")
        else:
            if "Disabled" in superkey_state :
               gsettings_reset( 'org.compiz.unityshell', 
                                '/org/compiz/profiles/unity/plugins/unityshell/',
                                'show-launcher')
            if "disabled" in altkey_state :
